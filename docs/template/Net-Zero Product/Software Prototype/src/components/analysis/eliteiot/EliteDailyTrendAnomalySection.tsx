...
import { EliteAnomalyDeviceDetailPanel } from "@/components/analysis/eliteiot/EliteAnomalyDeviceDetailPanel";
import {
...
import {
  asEliteData,
  getEliteScopedBaseline,
  getEliteScopedDailyHourly,
  getEliteScopedDailyTotals,
  getEliteScopedProfileHourly
} from "@/components/analysis/eliteiot/eliteiotScopeHelpers";
import type { EliteDailyCategorySplit } from "@/mock/eliteiotEnergyAnalysisData";
import { NapDailyTotalRow, NapEnergyAnalysisData, NapHourlyRow } from "@/mock/napEnergyAnalysisData";

type CategoryScope = EliteCategoryScope;
...
export type EliteDailyTrendViewMode = "full" | "trendOnly" | "anomalyListOnly";
...
const CATEGORY_OPTIONS = ELITE_CATEGORY_OPTIONS;
...
interface EliteDailyTrendAnomalySectionProps {
  data: NapEnergyAnalysisData;
  viewMode?: EliteDailyTrendViewMode;
  spaceScope?: CategoryScope;
  onSpaceScopeChange?: (scope: CategoryScope) => void;
...
interface TrendRow {
  ...
  holidayName?: string;
  total: number;
  categorySplit?: EliteDailyCategorySplit;
  expected: number;
  threshold: number;
  deltaPct: number;
  anomaly: boolean;
...
function buildScopedTrendRows(
  ...
  const dailyTotals = getEliteScopedDailyTotals(eliteData, scope);
  const typeMeans = getEliteScopedBaseline(eliteData, scope);
  ...
  return dailyTotals.map((row) => {
    ...
    return {
      ...
      total,
      categorySplit: eliteData.eliteDailyCategorySplits[row.date],
      expected,
      threshold,
      deltaPct,
      anomaly
...
export function EliteDailyTrendAnomalySection({
  data,
  viewMode = "full",
...
}: EliteDailyTrendAnomalySectionProps) {
  const unitLabel = "kWh";
  const hourlyUnitLabel = "kWh/h";
  const [internalSpaceScope, setInternalSpaceScope] = useState<CategoryScope>("incoming");
  ...
  const scopedTrendRows = useMemo(
    () => buildScopedTrendRows(data, spaceScope),
    [data, spaceScope]
  );
  ...
  const dayTypeBaseline = getEliteScopedBaseline(asEliteData(data), spaceScope)[dayTypeFilter];
  ...
  const anomalyDetailData = useMemo(() => {
    ...
    const selectedHourly =
      spaceScope === "incoming"
        ? (data.dailyHourlyBySpace.all[selectedAnomalyRow.dateLabel] ?? [])
        : getEliteScopedDailyHourly(eliteData, spaceScope, selectedAnomalyRow.dateLabel);
    const referenceHourly =
      spaceScope === "incoming"
        ? (data.profileHourlyBySpace.all[selectedAnomalyRow.dayType] ?? [])
        : getEliteScopedProfileHourly(eliteData, spaceScope, selectedAnomalyRow.dayType);
  ...
  const scopeLabel = eliteCategoryScopeLabel(spaceScope);
  const dayTypeOptions = useMemo(() => {
    if (!dayTypeFilterEnabled || data.profileMeta.holidayCount > 0) {
      return DAY_TYPE_OPTIONS;
    }
    return DAY_TYPE_OPTIONS.filter((option) => option.key !== "holiday");
  }, [dayTypeFilterEnabled, data.profileMeta.holidayCount]);
  ...
  return (
    <section className="panel p-4" id={showAnomalyList && !showTrend ? "elite-anomaly-section" : undefined}>
      {showTrend ? (
        <>
        ...
        <div>
          <RequirementGuideTitle
            ...
            content={{
              ...
              dataAcquisition: [
                "Chart data: incoming meter daily totals (dailyTotals) for the monitoring window.",
                "Expected baseline = day-type mean calibrated over the same EliteIOT monitoring window.",
                "Each day classified as weekday, weekend, or holiday with Malaysia holiday mapping (17 Jun 2026).",
              ...
              chartGeneration: [
                ...
                "Red ReferenceDot on anomaly days; category filter for Incoming / F&B / Lighting / IT Devices / General Plug.",
      ...
      <div className="mb-3 flex flex-wrap items-start gap-3">
        ...
        <div className="rounded-md border border-shell-600 bg-shell-900 p-2">
          <p className="mb-2 text-[11px] text-slate-400">Category Filter</p>
          <div className="inline-flex flex-wrap rounded border border-shell-600 bg-shell-800 p-1">
      ...
      <div className="rounded-md border border-shell-600" id={showTrend ? "elite-anomaly-section" : undefined}>
          ...
          <RequirementGuideTitle
            ...
            content={{
              ...
              dataAcquisition: [
                ...
                `Columns: date, day type, actual vs expected, threshold, sub-meter category split (incoming scope), delta %.`
              ],
              chartGeneration: [
            ...
            <table className="w-full text-xs">
              <thead className="bg-shell-700 text-slate-300">
                <tr>
                  ...
                  {spaceScope === "incoming" ? (
                    <th className="px-3 py-2 text-right">F&B / Light / IT / Plug (kWh)</th>
                  ) : null}
              ...
              <tbody>
                {anomalyRows.map((row) => (
                  ...
                  >
                    ...
                    {spaceScope === "incoming" && row.categorySplit ? (
                      <td className="whitespace-nowrap px-3 py-2 text-right text-slate-400">
                        {row.categorySplit.fnb.toFixed(1)} / {row.categorySplit.lighting.toFixed(1)} /{" "}
                        {row.categorySplit.it_devices.toFixed(1)} / {row.categorySplit.general_plug.toFixed(1)}
                      </td>
                    ) : null}
