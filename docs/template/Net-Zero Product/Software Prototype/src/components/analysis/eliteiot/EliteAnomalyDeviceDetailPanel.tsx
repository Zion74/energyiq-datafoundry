...
export function EliteAnomalyDeviceDetailPanel({
  ...
  detailMode,
  onDetailModeChange
}: EliteAnomalyDeviceDetailPanelProps) {
  ...
  const matrixRows = useMemo(() => {
    ...
    const allowedDevices = isEliteSubmeterScope(categoryScope)
      ? getEliteScopeDevices(eliteData, categoryScope)
      : null;
    return buildEliteDeviceHourlyMatrix(spikeProfiles, averageProfiles, allowedDevices);
  }, [categoryScope, data, dateLabel, dayType]);
  ...
  function renderDeviceLabel(row: (typeof matrixRows)[number]) {
    return (
      <div className="flex min-w-0 items-center gap-1">
        <EliteCategoryIcon category={row.category} />
        <span className="truncate" title={`${row.device} (${row.category})`}>
          {row.shortName}
        </span>
      </div>
    );
  ...
  return (
    <section className="rounded-md border border-shell-700 bg-shell-900 p-3">
      <h4 className="mb-2 text-xs font-semibold text-emerald-300">
        Sub-meter heatmap ({eliteCategoryScopeLabel(categoryScope)}): {shortLabel} vs {referenceLabel}
      </h4>
      ...
      {matrixRows.length === 0 ? (
        ...
        <div className="overflow-x-auto rounded-md border border-shell-700 bg-black/20 p-1.5">
          ...
          >
            <div className="sticky left-0 bg-shell-900 text-[9px] text-slate-500">Device</div>
