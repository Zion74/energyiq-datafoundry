...
interface NapLevelUsageHeatmapSectionProps {
...
function profileLevel(profile: NapDeviceHourlyProfile): 6 | 7 {
...
export function NapLevelUsageHeatmapSection({ data }: NapLevelUsageHeatmapSectionProps) {
  const [profileType, setProfileType] = useState<ProfileType>("weekday");
  const [selectedLevel, setSelectedLevel] = useState<6 | 7 | null>(null);
  ...
  const levelBreakdownRows = useMemo((): LevelBreakdownRow[] => {
    const totalKwh = levelUsage.level6Kwh + levelUsage.level7Kwh;
    return ([6, 7] as const).map((level) => {
      const profiles = deviceHourlyProfiles.filter((profile) => profileLevel(profile) === level);
      const levelTotal = level === 6 ? levelUsage.level6Kwh : levelUsage.level7Kwh;
      const aggregatedHourly = aggregateLevelHourly(profiles);
      const peak = peakFromHourly(aggregatedHourly);
      return {
        level,
        label: `Level ${level}`,
        totalKwh: levelTotal,
        sharePct: totalKwh > 0 ? (levelTotal / totalKwh) * 100 : 0,
        dailyAvgKwh:
          levelUsage.sampleCount > 0
            ? Math.round((levelTotal / levelUsage.sampleCount) * 10) / 10
            : 0,
  ...
  }, [deviceHourlyProfiles, levelUsage.level6Kwh, levelUsage.level7Kwh, levelUsage.sampleCount]);

  const selectedProfiles = useMemo(() => {
    if (selectedLevel == null) {
      return [];
    }
    return deviceHourlyProfiles.filter((profile) => profileLevel(profile) === selectedLevel);
  }, [deviceHourlyProfiles, selectedLevel]);
  ...
  const heatmapColorBounds = useMemo(() => {
    ...
    const levelProfiles = PROFILE_OPTIONS.flatMap(({ key }) =>
      (data.deviceHourlyProfilesByDayType[key] ?? []).filter(
        (profile) => profileLevel(profile) === selectedLevel
      )
    );
    return computeHeatmapBounds(collectHourlyValues(levelProfiles));
  }, [data.deviceHourlyProfilesByDayType, selectedLevel]);

  const selectedLevelLabel = selectedLevel ? `Level ${selectedLevel}` : null;
  const showHeatmap = selectedLevel != null && selectedProfiles.length > 0;

  return (
    <section className="mt-4 rounded-md border border-shell-700 bg-black/10 p-3">
      <RequirementGuideTitle
        title="Daily Usage Pattern by Level"
        className="mb-2 text-xs font-semibold text-white"
        content={{
          title: "Daily Usage Pattern by Level",
          summary: "Level summary and sub-meter hourly heatmap by day type (weekday / weekend / holiday).",
          dataAcquisition: [
            ...
            "Level table: share of level total, daily average (kWh/day), and peak clock hour with intensity."
          ],
          chartGeneration: [
            "Selectable level breakdown table; sub-meter detail list for the chosen level.",
            "Heatmap appears after level selection; one row per sub-meter, 24 clock hours.",
            "Colour scale (kWh/h) is fixed per level and shared across weekday, weekend, and holiday views."
          ]
        }}
      />
      <div className="mb-3 flex flex-wrap items-center justify-between gap-2">
        <p className="text-[11px] text-slate-400">
          Average hourly usage · Unit kWh/h · {profileSampleLabel} · Select a level to view sub-meter heatmap
        </p>
          ...
          <div className="min-h-0 shrink-0 overflow-x-auto">
            <table className="w-full min-w-[380px] table-fixed text-[11px]">
              ...
              <thead className="bg-shell-700 text-slate-300">
                <tr>
                  <th className="whitespace-nowrap px-2 py-1.5 text-left">Level</th>
                  <th className="whitespace-nowrap px-2 py-1.5 text-left">Total (kWh)</th>
              ...
              <tbody>
                {levelBreakdownRows.map((row) => (
                  <tr
                    key={row.level}
                    className={`cursor-pointer border-t border-shell-600 ${
                      selectedLevel === row.level ? "bg-blue-500/10 text-blue-200" : "text-slate-200 hover:bg-shell-800"
                    }`}
                    onClick={() => setSelectedLevel(row.level)}
                  >
                    <td className="whitespace-nowrap px-2 py-1.5">{row.label}</td>
                    <td className="whitespace-nowrap px-2 py-1.5">{row.totalKwh.toLocaleString(undefined, { maximumFractionDigits: 1 })}</td>
            ...
            <div className="min-h-[240px] flex-1 border-t border-shell-600">
              <div className="border-b border-shell-700 bg-shell-900 px-3 py-1.5 text-[10px] text-slate-400">
                {selectedLevelLabel
                  ? `${selectedLevelLabel} sub-meters (${deviceDetailRows.length})`
                  : "Sub-meter details"}
              </div>
              <div>
                {selectedLevel == null ? (
                  <p className="px-3 py-4 text-[11px] text-slate-500">Select a level to view sub-meter details</p>
                ) : (
          ...
          <div className="mb-2 flex shrink-0 items-center justify-between gap-2">
            ...
            {selectedLevelLabel ? (
              <p className="text-[10px] text-slate-500">
                Selected: <span className="text-slate-300">{selectedLevelLabel}</span>
              </p>
            ) : null}
          </div>
          ...
          <div className="flex min-h-0 flex-1 flex-col justify-center pt-3">
            {!showHeatmap ? (
              <div className="flex items-center justify-center rounded-md border border-dashed border-shell-600 px-4 py-16 text-xs text-slate-500">
                Select a level from the left table to display heatmap
              </div>
            ) : (
