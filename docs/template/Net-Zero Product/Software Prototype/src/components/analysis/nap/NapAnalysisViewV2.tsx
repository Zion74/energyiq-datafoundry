...
import { NapConsumptionBreakdownChart } from "@/components/analysis/nap/NapConsumptionBreakdownChart";
...
type TrendSpaceScope = "all" | "level6" | "level7";

interface NapAnalysisViewV2Props {
  data: NapEnergyAnalysisData;
}

/** v2.0 layout — Daily Total Trend, anomaly count, and Detected Anomaly List pinned above Executive Summary. */
export function NapAnalysisViewV2({ data }: NapAnalysisViewV2Props) {
  const topPeaks = useMemo(() => resolveTopPeaks(data.topPeaks), [data.topPeaks]);
  const [trendSpaceScope, setTrendSpaceScope] = useState<TrendSpaceScope>("all");
  ...
  return (
    <>
      ...
      <AnalysisSection
        ...
        subtitle="Day-type energy health indicators for Level 6 and Level 7 aggregate meters."
